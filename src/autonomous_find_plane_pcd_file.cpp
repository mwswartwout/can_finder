...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < downsampled_kinect_ptr->points.size(); i++) {
        double x = downsampled_kinect_ptr->points[i].x;
        double y = downsampled_kinect_ptr->points[i].y;
        double z = downsampled_kinect_ptr->points[i].z;
        ...
        if (x_valid && y_valid && z_valid) {
            ...
            new_point.y = y;
            new_point.z = z;
            selected_pts_cloud_ptr->push_back(new_point);
        }
    }
